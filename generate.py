...
def main(arguments):
    ...
    L = my_sentence.split()
    n = random.randint(0,int(len(L)/3))# No. of words to get masked
    id = random.sample(range(0, len(L)), n+1) # id of words in sentence to get masked
    for i in id:
        L[i] = "[MASK]"
    my_sentence = " ".join([".","."]+L+[".","."])
    args.seed_sentence=my_sentence
